...
async function connectMe(_provider) {
    try {
        ...
        if(!_comn_res) {
          ...
          // Shorten the selected account address
let shortenedAddress = selectedAccount.slice(0, 6) + '...' + selectedAccount.slice(-6);

// Set the shortened address in the HTML element
$('#selected-address').html(`Address: ${shortenedAddress}`);
